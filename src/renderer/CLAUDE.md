...
- **TTSManager.tsx** — Headless component, Web Speech API (it-IT), priority queue, P1 interrupts. Used for real-time lap alerts when Azure TTS is not enabled. Also drains the one-shot `ipcStore.announce` slot (P2): session-open outcome and the `<sintesi-vocale>` extract spoken when a Level-1 analysis completes
